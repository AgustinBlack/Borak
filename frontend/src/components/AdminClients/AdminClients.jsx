...
// >>>>>>> efa84546fe0134dfd1c852255dff924c2612feff
//     }
//   };
...
const SECCIONES = ["movilidad", "zona media", "circuito", "fisico"];

const emptyExercise = () => ({
  name: "", type: "reps", series: "", reps: "", duration: "", weight: ""
});

const emptyDay = (index) => ({
  name: `Día ${index + 1}`,
  weekDay: null,
  secciones: { movilidad: [], "zona media": [], circuito: [], fisico: [] }
});
...
const AdminClients = ({ user }) => {
  ...
  // Auto-refresh cada 30 segundos (de tu amigo)
  useEffect(() => {
    if (user?.role !== "pf") return;
  ...
  // Notificación de nuevos pendientes (de tu amigo)
  useEffect(() => {
    if (pendingUsers.length > prevPendingCount && prevPendingCount !== 0) {
      const newest = pendingUsers[pendingUsers.length - 1];
      setNewPendingName(newest?.name || "Un usuario");
  ...
  const handleCreateExercise = async () => {
    try {
      if (!newExerciseName.trim()) return alert("Poné un nombre");
      const res = await fetch("http://localhost:3000/exercises", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: newExerciseName, video_url: newExerciseVideo })
      });
      const data = await res.json();
  ...
  // CREATE ROUTINE
  // =====================
  const addDay = () => setRoutineDays([...routineDays, emptyDay(routineDays.length)]);

  const removeDay = (dayIndex) => setRoutineDays(prev => prev.filter((_, i) => i !== dayIndex));
  ...
  const addExerciseToSection = (dayIndex, seccion) => {
    const updated = [...routineDays];
    updated[dayIndex].secciones[seccion].push(emptyExercise());
    setRoutineDays(updated);
  };

  const removeExerciseFromSection = (dayIndex, seccion, exIndex) => {
    const updated = [...routineDays];
    updated[dayIndex].secciones[seccion] = updated[dayIndex].secciones[seccion].filter((_, i) => i !== exIndex);
    setRoutineDays(updated);
  };

  const updateExerciseInSection = (dayIndex, seccion, exIndex, field, value) => {
    const updated = [...routineDays];
    updated[dayIndex].secciones[seccion][exIndex] = {
      ...updated[dayIndex].secciones[seccion][exIndex],
      [field]: value
    };
    setRoutineDays(updated);
      ...
      exercises: SECCIONES.flatMap(sec =>
        (day.secciones?.[sec] || [])
          ...
          .map(ex => ({
            ...
            type: ex.type,
            series: Number(ex.series) || 0,
            reps: ex.type === "reps" ? Number(ex.reps) || 0 : 0,
            duration: ex.type === "tiempo" ? Number(ex.duration) || 0 : 0,
            weight: Number(ex.weight) || 0
          }))
      )
    }));
  ...
  // =====================
  const handleSaveRoutine = async () => {
    if (!targetClient) return alert("Seleccioná un cliente");
    if (!newRutineName.trim()) return alert("Poné un nombre");
    if (routineDays.length === 0) return alert("Agregá al menos un día");
    if (routineDays.some(day => day.weekDay === null)) return alert("Todos los días deben tener un día asignado");

    try {
      const res = await fetch("http://localhost:3000/assign-routine", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId: targetClient, routineName: newRutineName, days: flattenDays(routineDays) })
      });
  ...
  const handleEditRoutine = async (userId) => {
    try {
      ...
      const data = await res.json();
      if (!res.ok) return alert(data.message || "No se encontró rutina");

      const daysWithSections = (data.days || []).map(day => {
        const secciones = { movilidad: [], "zona media": [], circuito: [], fisico: [] };
        (day.exercises || []).forEach(ex => {
          const sec = ex.section || "fisico";
          if (secciones[sec]) {
        ...
        return { name: day.name, weekDay: day.weekDay, secciones };
      });
  ...
  const updateEditingDayField = (dayIndex, field, value) => {
    const updated = [...editingRoutineDays];
    updated[dayIndex] = { ...updated[dayIndex], [field]: value };
    setEditingRoutineDays(updated);
  };

  const addEditingExercise = (dayIndex, seccion) => {
    const updated = [...editingRoutineDays];
    updated[dayIndex].secciones[seccion].push(emptyExercise());
    setEditingRoutineDays(updated);
  };

  const removeEditingExercise = (dayIndex, seccion, exIndex) => {
    const updated = [...editingRoutineDays];
    updated[dayIndex].secciones[seccion] = updated[dayIndex].secciones[seccion].filter((_, i) => i !== exIndex);
    setEditingRoutineDays(updated);
  };

  const updateEditingExercise = (dayIndex, seccion, exIndex, field, value) => {
    const updated = [...editingRoutineDays];
    updated[dayIndex].secciones[seccion][exIndex] = {
      ...updated[dayIndex].secciones[seccion][exIndex],
      [field]: value
  ...
  const handleAddEditingDay = () => {
    setEditingRoutineDays([...editingRoutineDays, emptyDay(editingRoutineDays.length)]);
  };
  ...
  const handleUpdateRoutine = async () => {
    try {
      await fetch(`http://localhost:3000/update-routine/${editingUserId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ routineName: editingRoutineName, days: flattenDays(editingRoutineDays) })
      });
      alert("Rutina actualizada 🔥");
      setShowEditModal(false);
      setEditingUserId(null);
  ...
  // RENDER SECCIONES
  // =====================
  const renderSecciones = (days, dayIndex, { updateExercise, addExercise, removeExercise, removeDay, updateDayField: updateField }) => (
    <div key={dayIndex} className={styles.exerciseList}>
      <div className={styles.formRow}>
        <div className={styles.formGroup}>
          ...
          <input
            value={days[dayIndex].name}
            onChange={(e) => updateField(dayIndex, "name", e.target.value)}
          />
        </div>
        <div className={styles.formGroup}>
          <label>Día de la semana</label>
          <select
            value={days[dayIndex].weekDay ?? ""}
            onChange={(e) => updateField(dayIndex, "weekDay", e.target.value === "" ? null : Number(e.target.value))}
          >
        ...
        <button className={styles.btnRemove} onClick={() => removeDay(dayIndex)}>✕</button>
      </div>

      {SECCIONES.map(seccion => (
        <div key={seccion} className={styles.seccionBlock}>
          <div className={styles.seccionHeader}>
            <h4 className={styles.seccionTitle}>
              {seccion.charAt(0).toUpperCase() + seccion.slice(1)}
            </h4>
            <button className={styles.btnAddExercise} onClick={() => addExercise(dayIndex, seccion)}>
              + Ejercicio
            </button>
          </div>

          {days[dayIndex].secciones[seccion].length === 0 && (
            <p className={styles.seccionVacia}>Sin ejercicios en esta sección</p>
          )}

          {days[dayIndex].secciones[seccion].map((ex, exIndex) => (
            <div key={exIndex} className={styles.exerciseAdder}>
              <select
                value={ex.name}
                onChange={(e) => {
                  if (e.target.value === "__new__") { setShowExerciseModal(true); return; }
                  updateExercise(dayIndex, seccion, exIndex, "name", e.target.value);
                }}
              >
                <option value="">Elegir ejercicio</option>
                <option value="__new__">➕ Agregar nuevo ejercicio</option>
                {exercises.map(ej => (
              ...
              <select
                value={ex.type}
                onChange={(e) => updateExercise(dayIndex, seccion, exIndex, "type", e.target.value)}
              >
                <option value="reps">Reps</option>
                <option value="tiempo">Tiempo</option>
              </select>

              <input type="number" placeholder="Series" value={ex.series}
                onChange={(e) => updateExercise(dayIndex, seccion, exIndex, "series", e.target.value)} />

              {ex.type === "reps" ? (
                <input type="number" placeholder="Reps" value={ex.reps}
                  onChange={(e) => updateExercise(dayIndex, seccion, exIndex, "reps", e.target.value)} />
              ) : (
                <input type="number" placeholder="Seg" value={ex.duration}
                  onChange={(e) => updateExercise(dayIndex, seccion, exIndex, "duration", e.target.value)} />
              )}

              <input type="number" placeholder="Kg" value={ex.weight}
                onChange={(e) => updateExercise(dayIndex, seccion, exIndex, "weight", e.target.value)} />

              <button className={styles.btnRemove} onClick={() => removeExercise(dayIndex, seccion, exIndex)}>✕</button>
            </div>
          ))}
        </div>
      ))}
      ...
      {showExerciseModal && (
        <div className={`${styles.modalCustom} ${styles.modalTop}`}>
          <div className={styles.modalContent}>
            ...
            <div className={styles.formGroup}>
              <label>Video URL (opcional)</label>
              <input value={newExerciseVideo} onChange={(e) => setNewExerciseVideo(e.target.value)} />
            </div>
            <div className={styles.modalActions}>
              <button className={styles.btnCancel} onClick={() => setShowExerciseModal(false)}>Cancelar</button>
              <button className={styles.btnSave} onClick={handleCreateExercise}>Guardar</button>
            </div>
          </div>
        </div>
      )}
          ...
          <tbody>
            {activeUsers.length > 0 ? activeUsers.map((u) => (
              <tr key={u.id} className={styles.clientRow}>
                ...
                <td data-label="Rutina">{u.current_routine || "Sin rutina"}</td>
                <td>
                  <button className={styles.btnAssign} onClick={() => { setTargetClient(u.id); setShowCreateModal(true); }}>
                    Asignar Rutina
                  </button>
                  <button className={styles.btnEdit} onClick={() => handleEditRoutine(u.id)}>
                    Modificar Rutina
                  </button>
                </td>
              </tr>
              ...
              <tr>
                <td colSpan="4" style={{ textAlign: "center" }}>No se encontraron alumnos activos.</td>
              </tr>
            )}
          </tbody>
        </table>
